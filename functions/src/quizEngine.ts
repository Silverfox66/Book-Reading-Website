/**
 * Deterministic quiz generator. Mirrors src/quizEngine.ts on the client, but this
 * copy is the source of truth: the server generates and grades quizzes so a
 * client can never see (or forge) the answer key ahead of time.
 */
...
function buildChoiceSet(correct: string, decoyPool: string[], rng: () => number) {
  const decoys = pickDecoys(decoyPool, correct, rng, 4);
  const choices = shuffleWithRng([correct, ...decoys], rng);
  return { choices, correctIndex: choices.indexOf(correct) };
}

// Random per-session salt (passed in by the caller) is mixed into the seed so a
// user cannot pre-compute the answer key just by knowing the deterministic seed.
export function generateQuizQuestions(bookId: string, sessionSalt: string, meta: BookMeta): GeneratedQuestion[] {
  const rng = mulberry32(hashSeed(bookId + '|' + sessionSalt + '|' + meta.title));
